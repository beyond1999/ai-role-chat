...
asr_model = WhisperModel("../../models/faster-whisper-small", device="cuda", compute_type="int8")
...
def test_time(file_name):
    audio_path = "../../data/sound/" + file_name
...
def test_cuda_occupation(file_name):
    audio_path = "../../data/sound/" + file_name
    if not os.path.exists(audio_path):
        print (f"音频文件不存在: {audio_path}")
        return
...
if __name__ == "__main__":
    ...
    while file_name != "exit" and file_name != "exit()":
        test_cuda_occupation(file_name)
        file_name = input("please enter the file name:")
